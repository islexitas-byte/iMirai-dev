...
client = OpenAI(api_key='',base_url='http://207.180.148.74:47046/v1')
...
def question_sql_genration(table_schema,status,data1):
    ...
    query = {}
    input_text = f"""
    Your task is follow the given instructions:
    -Given the following table schema, identify the key columns, and ask questions that a data analyst might ask to derive meaningful insights.
    -Remember, you must not ask more than 15 questions. It's a tumb rule.
    -The output must exclusively contain questions, separated by '\n' (this format must be strictly adhered to)(e.g., Question1\nQuestion2\nQuestion3).
    Table Schema:{table_schema}
    """
    if status==True:
        input_text1 = f"""
        Your task is follow the given instructions:
        -Given the following table schema and data in back ticks, generate questions that leverage time-based analysis, such as trends, seasonality, and comparisons over time
        -If you want to generate for specific years, the data below provides information on the available years in the dataset.
        -Remember, you must not ask more than 15 questions. It's a tumb rule
        -The output must exclusively contain questions, separated by '\n' (this format must be strictly adhered to)(e.g., Question1\nQuestion2\nQuestion3).
        Table Schema:`{table_schema}`
        Data:`{data1}`
...
def convert_numeric(data):
    ...
    for col in data.select_dtypes(include=['int64','float64']).columns:
        data[col].fillna(0.0,inplace=True)
    return data
def load_data(data_table,table_name,user_name,content_id,session_id,file_path):
    ...
    table_columns = ', '.join(
    ['"{}" {}'.format(col.upper(), types[str(original_types[col])]) for col in create_columns]
    )
    ...
    f'oracle+oracledb://{USER_NAME}:{PASSWORD}@{HOST}:{PORT}/?service_name={SERVICE_NAME}',
    include_tables=[table_name.lower()])
    ...
    cursor.execute(f"DELETE FROM FILE_PATH_LIST WHERE SESSION_ID=:sessionId",sessionId=session_id)
