...
def weeks_in_month(year, month):
    ...
    return 5 if last_day > 28 else 4

def week_of_month(date):
    return (date.day - 1) // 7  # 0-based
# ============== End ====================
...
def save_project_plans(project_plan):
    try:
        ...
        for month_name, _ in months:
            ...
            total_weeks += weeks_in_month(2026, month_dt.month)
        # ========= END ========================
        ...
        for month, weeks in months:
            ...
            weeks = weeks_in_month(2026, month_dt.month)
            # ========= END =============
        ...
        # ============ LEGEND COLOR MAIN CODE =================
        start_week_fill = PatternFill(start_color="F4B183", end_color="F4B183", fill_type="solid")  # Orange
        end_week_fill   = PatternFill(start_color="00B050", end_color="00B050", fill_type="solid")  # Green

        # --- START / END WEEK COLORING (CALENDAR-BASED) ---

        # Build month → starting column mapping
        month_start_col = {}
        col_ptr = start_col

        for month_name, _ in months:
            month_dt = datetime.strptime(month_name, "%B")
            w_count = weeks_in_month(2026, month_dt.month)
            month_start_col[month_dt.month] = col_ptr
            col_ptr += w_count

        for r_idx, row in enumerate(given_plan.itertuples(index=False), start=DATA_START_ROW):

            try:
                start_date = pd.to_datetime(row[given_plan.columns.get_loc("Start Date (W)")])
                end_date   = pd.to_datetime(row[given_plan.columns.get_loc("End Date (W)")])
            except Exception:
                continue
            ...
            # --- START WEEK ---
            sm = start_date.month
            sw = week_of_month(start_date)
            start_col_idx = month_start_col[sm] + sw

            # --- END WEEK ---
            em = end_date.month
            ew = week_of_month(end_date)
            end_col_idx = month_start_col[em] + ew

            # Apply END first
            cell = ws.cell(row=r_idx, column=end_col_idx)
            cell.fill = end_week_fill
            cell.border = thin_border

            # Apply START last (orange overrides if same cell)
            cell = ws.cell(row=r_idx, column=start_col_idx)
            cell.fill = start_week_fill
            cell.border = thin_border
        # ================= END ====================
